...
@foreach($categories as $category )                                                <!-- creating instance throught request method of category and comapring slug  -->
<a href="{{route('by-category', $category)}}" class="text-semibold block py-2 px-3 {{request('category')?->slug === $category->slug ? 'bg-blue-600 text-white' : ''}}">
{{$category->title}}
{{$category->total}}
</a>
@endforeach
